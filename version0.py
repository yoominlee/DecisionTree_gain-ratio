## version 0은 현재 train.txt 쌍들에서는 잘 작동함
## 다만 train1.txt 오류 나는 상황.
...
##      %%%%%%%%%%%%%%%%%%%%        !!  Node class 생성하는 부분 파라미터들 전체적으로 확인
class Node:
    def __init__(self, data, feature, is_leaf=False):
        # parent가 []이면 root로?
...
def make_node(data, used_feature):
    ...
    for i in feature_val[select]:
        ...
        if len(partition_data)>=1:
            ...
            node.child[i] = Node(data=partition_data, feature=used_feature, is_leaf=True)
        count += 1
    return node
...
def gain_compare(data, used_feature):
    ...
    for i in range(len(feature) - 1):
        if used_feature!=None:
            if i in used_feature: gain_compare.append(0) # used_feature에 뭔가가 있고, i가 거기에 있을때 (안사용하겠다)
